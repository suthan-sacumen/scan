...
pa=~/codeBase/gitLabDemo/securityscorescard
prisma_cloud_access_key=f16cc26c-6e9e-4b41-a713-d82d1f6344f9
prisma_cloud_api_url=https://api.gov.prismacloud.io
prisma_cloud_scm_asset_name=test_gitlab_webhook
prisma_cloud_scm_failure_issue_criteria='High: 1000, Medium: 1000, Low: 1000, Operator: AND'
prisma_cloud_scm_failure_mr_criteria='High: 1000, Medium: 1000, Low: 1000, Operator: AND'
prisma_cloud_scm_tags='owner:mr.test, Team:SHIFT_LEFT'
prisma_cloud_secret_key=IfUM1wsBXej7PUB7rZ1+yT0Bkso=
prisma_cloud_cicd_asset_name=test
if [[ "$pa" != "" ]];then
    repo_path=$pa
else
...
cd $repo_path
#ls -al .
zip -r $repo_path/iacscan.zip . -x \*.git\* #here cd inside repo_path and '.' as source is mandatory else while zipping copies else it will zip from root instead of files inside repo
